...
class FaceDetector:
    def __init__(self, onnx_file, rac):
    ...
    def prepare(self, use_gpu, nms=0.4, fix_image_size=None, ctx=0):
        ...
        if use_gpu:
            ...
            self.ort_session = onnxruntime.InferenceSession(self.onnx_file, sess_options=sessionOptions)
            print("det intra_op_num_threads {} inter_op_num_threads {}".format(sessionOptions.intra_op_num_threads, sessionOptions.inter_op_num_threads))



        self.input_name = self.ort_session.get_inputs()[0].name  # 'data'

        self.outputs = [self.ort_session.get_outputs()[i].name for i in range(len(self.ort_session.get_outputs()))]
        # print(len(self.ort_session.get_outputs()))
        # print(self.outputs)
        # return
        img = np.zeros((3,224,224))
        input_blob = np.expand_dims(img, axis=0).astype(np.float32)  # NCHW

        out = self.ort_session.run(self.outputs, input_feed={self.input_name: input_blob})
    ...
    def detect(self, img, threshold=0.5, scale=1.0, ret_landmark=True):
        proposals_list = []
        scores_list = []
        landmarks_list = []
        if scale==1.0:
        ...
        net_out = self.ort_session.run(self.outputs, input_feed={self.input_name: im_tensor})
        ...
        proposals = proposals[order, :]
        scores = scores[order]
        if self.use_landmarks and ret_landmark:
...
def get_retinaface(_file, rac='net3'):
